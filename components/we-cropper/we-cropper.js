...
function prepare () {
  ...
  self.createCtx = function () {
    var id = self.id;
    if (id) {
      self.ctx = my.createCanvasContext(id);
    } else {
...
function methods () {
  ...
  self.getCropperImage = function () {
    console.warn("裁剪",arguments,self.ctx);
    ...
    var quality = ref.quality; if ( quality === void 0 ) quality = 10;
        ...
        self.ctx.toTempFilePath({
          ...
          success: function success (res) {
            isFunction(fn) && fn.call(self, res.apFilePath);
          },
          fail: function fail (res) {
